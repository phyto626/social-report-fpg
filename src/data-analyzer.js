...
class DataAnalyzer {
  constructor(data, brandKey) {
    this.pageInfo = data.pageInfo;
    this.posts = data.posts;
    this.startDate = data.startDate;
    this.endDate = data.endDate;
    // brandKey 優先判斷品牌；若未傳入則 fallback 至 pageInfo.name
  ...
  generateInsights(topicAnalysis, mediaAnalysis) {
    ...
    // 1. 內容結構 (問答、條列)
    const isEcoco = this.brandKey === 'ecoco' ||
      (this.pageInfo && (
        this.pageInfo.name.toLowerCase().includes('ecoco') ||
        this.pageInfo.name.toLowerCase().includes('eco')
      ));
  ...
  generateActivityPlan(topicAnalysis, mediaAnalysis) {
    ...
    const brandDisplayName = (this.pageInfo && this.pageInfo.name) || '台塑便利家';
    const m = this.month;
    ...
    plan.push({
      ...
      benefit: isEcoco 
        ...
        : `以${painPointsMap[h1.seasonId]}為切入點，自然點出${brandDisplayName}環保洗劑（或補充站）的便利與無殘留特性，產生生活共鳴。`
    });
